...
import ProductCreate from "../../components/product_create/PurchseEntryCreate";
...
const EmployeeCreate = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `purchase_entry_dispatch_get?access_token=${access_token}&dispatch_start_date=${start_date}&dispatch_end_date=${end_date}&dispatch_order_number=${dispatch_order_number}&page=${limit}&items_per_page=${items_per_page}`
      )
      .then((response) => {
          ...
          res.Purchase_Materials_Data.map((val) => {
            const invoice_material = {
              ...
              additional_cess_rate:
                val.additional_cess_value / val.material_qty,
  ...
  const HandleAddBatch = (index, ordindex) => {
    const newField = [...invoiceDetails];
    const order_material = newField[ordindex].order_material;
    if (order_material[index].material_name !== "") {
      setmaterialDataIndex(index);
      setmaterialDataBool(true);
      setmaterialData(order_material[index]);
    } else {
  ...
  const product_table = [
    ...
    { th: "Balance Quantity", id: "balance_qty", weigh: "6%", sub_item: [] },
          ...
          if (res.data.action == "success") {
            ...
            if (
              ...
              const order_material =
                order_material_list[ordindex].order_material;
  ...
  const EmployeeCreateMaster = async () => {
    ...
    const mdata = {
      ...
      method_type: "dispatch_order",
      supplier_id: supplierID,
      supplier_name: supplierName,
      supplier_data: supplierDataValue,
      dispatch_order_id: DISPATCH_NUMBER,
      do_no: dispatchNumber,
  ...
  const updatedInvoiceDetails = invoiceDetails.map((vale) => {
    let totalPayable = 0;
    vale.order_material.forEach((item) => {
      totalPayable += item.total_payable || 0;
    });
    return { ...vale, total_amount: totalPayable };
  });
