          ...
          response.data.data[0]?.invoice_material_details.map((item) => {
            if (item.balance_qty !== 0) {
              const order_list = {
                ...
                new_batch_value: item.new_batch_value === undefined ? "" : item.new_batch_value,
                hsn_code: item.hsn_code,
                material_qty: item.material_qty,
                base_value: item.base_value,
                invoice_discount: item.invoice_discount,
                other_discount: item.other_discount,
                invoice_discount_per: item.invoice_discount_value === undefined ? '' : item.invoice_discount_value,
                other_discount_per: item.invoice_discount_value === undefined ? "" : item.invoice_discount_value,
      ...
      <Box sx={{ margin: "8px" }}>
        <Box sx={{ height: "83vh", overflow: "auto", padding: "8px" }}>
          <>
            ...
            <Paper sx={{ p: 1, mb: 1 }}>
              <VehicleCreate vehicleDetails={vehicleDetails} HandleVehicleDetails={HandleVehicleDetails} />
              <Box
                className="master_create_style_withOut_P"
                sx={{
                  px: 1,
