...
import ProductCreate from "../../components/product_view/ReportCreate";
import PurchaseItemReport from "../../components/product_view/PurchaseItemReport";
...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `dispatch_do_reports_get?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&supplier_id=${supplier_id}&invoice_date=${invoice_date}`
      )
      .then((response) => {
  ...
  const handleInvoiceTabTwoChange = (event, value) => {
    if (value != null) {
      setinvoiceTabTwoName(value.pur_invoice_number);
    } else {
            ...
            <Stack direction={"row"} gap={1}>
              ...
              <Box sx={{ mt: 1 }}>
                <Autocomplete
                  ...
                  value={
                    invoiceData.find(
                      (year) => year.pur_invoice_number === invoiceTabTwoName
                    ) || null
                  }
                  onChange={(e, value) =>
