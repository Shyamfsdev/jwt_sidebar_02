...
import ProductCreate from "../../components/product_view/EntryReport";
import PurchaseItemReport from "../../components/product_view/EntryItemReport";
...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `entry_reports_get?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&supplier_id=${supplier_id}&invoice_date=${invoice_date}`
      )
      .then((response) => {
  ...
  const HandlePurchaseInvoice = () => {
    const fetchData = async (
      ...
      axiosGet
        .get(
          `purchase_entry_master_list?access_token=${access_token}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}`
        )
        .then((response) => {
  ...
  const product_table = [
    ...
    { th: "GST Number", id: "pur_invoice_status", weigh: "12%", sub_item: [] },
    ...
    { th: "Entry Number", id: "additional_cess_value", weigh: "6%", sub_item: [] },
    { th: "Entry Date", id: "invoice_date", weigh: "8%", sub_item: [] },
  ...
  const handleInvoiceTabTwoDateChange = (event) => {
    const formattedDate = moment(event.target.value).format("YYYY-MM-DD");
    setinvoiceTabTwoDate(formattedDate);
  };

  const product_table_purchase_item = [
    { th: "#", id: "id", weigh: "2%", sub_item: [] },
    { th: "Supplier Name", id: "pur_invoice_material_id", weigh: "15%", sub_item: [] },
    ...
    { th: "Entry Number", id: "pur_entry_number", weigh: "6%", sub_item: [] },
  ...
  const [dateItemTitle, setDateItemTitle] = useState("Entry Date");
    ...
    if (tabsValue === 1) {
      return (
        <Box className="reports_bax_sub">
          <Stack direction={"row"} gap={1} sx={{ padding: "10px" }} className="displey_space_between">
