...
import ProductCreate from "../../components/product_view/SortageReport";
import PurchaseItemReport from "../../components/product_view/SortageItem";
...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `mismatch_report?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&supplier_id=${supplier_id}&pur_entry_number=${pur_entry_number}&inwaed_entry_method=${1}&`
      )
      .then((response) => {
  ...
  const HandlePurchaseInvoice = () => {
    const fetchData = async (
      ...
      axiosGet
        .get(
          `mismatch_report?access_token=${access_token}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&inwaed_entry_method=${1}`
        )
        .then((response) => {
  ...
  const product_table_purchase_item = [
    ...
    { th: "Shortage Number", id: "tcs_value", weigh: "6%", sub_item: [] },
  ...
  const tabs = [
    {
      value: 1,
      name: "Shortage Reports",
      content: <TabContext value={1} />,
    },
    {
      value: 2,
      name: "Shortage Item Reports",
