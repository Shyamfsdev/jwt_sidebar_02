...
const OrderList = () => {
    ...
    const fetchData = async (
        ...
        axiosGet
            .get(
                `claims_ccis_list?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&claims_ccis_type=${1}`
            )
            .then((response) => {
    ...
    data?.map((item, index) => {
        const array_data = {
            ...
            data: [
                ...
                {
                    td: moment(item.claims_date).format("YYYY-MM-DD"),
                    type: "text",
                    id: 2,
                    alian: "left",
                },
            ...
            <Box sx={{ px: 2, height: "1" }}>
                <div className="dispatch_do_flex">
                    <div className="display_flex global_padding" style={{ width: '50%' }}>
                        <Typography variant="h4" className="nunito_font" style={{ fontSize: '16px', fontWeight: "700", color: '#185AA6' }}>Claims K4 [{dataCount}] </Typography>
                    </div>
