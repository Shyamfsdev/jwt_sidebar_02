...
const EmployeeCreate = () => {
    ...
    const handleSalesInvoiceChange = (event, value) => {
        ...
        if (value != null) {
            ...
            setsupplierDetails(value.customer_data);
            setsupplierID(value.customer_id);
            setsupplierName(value.customer_name);
    ...
    const totalSum = orderMaterials?.reduce((sum, item) => sum + item.trade_discount_value, 0);

    const totalPer = orderMaterials?.reduce((sum, item) => sum + item.trade_discount, 0);
    ...
    const EmployeeCreateMaster = async () => {
        ...
        const mdata = {
            ...
            sales_number: salesInvoiceNumber,
            sales_id: salesInvoiceID,
            customer_id: supplierID,
            customer_name: supplierName
        };
        console.log(mdata);
        axiosPost.post(`claims_non_ccis_api`, mdata).then((res) => {
            ...
            if (res.data.action_status === "Error") {
                ...
                setTimeout(() => {
                    router.push("/claims_non_ccis_list");
                }, 200);
            }
        });
    };
            ...
            >
                ...
                {salesInvoiceNumber !== "" && <div style={{ borderRadius: "10px", border: "2px solid rgb(222 222 222)" }}><Box sx={{ padding: "8px 10px" }}>
                    <Typography className="table_cell_2">
                        {supplierName} ( {supplierDetails?.contact_number} ),
                    </Typography>
                    <Typography className="table_cell_2">
