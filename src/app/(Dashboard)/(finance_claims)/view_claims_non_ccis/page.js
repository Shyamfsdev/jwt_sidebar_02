...
const EmployeeCreate = () => {
    ...
    const GetFetData = () => {
        ...
        const fetchData = async (access_token, data_uniq_id) => {
            axiosGet
                .get(
                    `claims_non_ccis_list?access_token=${access_token}&data_uniq_id=${data_uniq_id}`
                )
                .then((response) => {
                    console.log(response);
                    setsupplierDetails(response.data.data[0]?.customer_list);
                    setsupplierName(response.data.data[0]?.customer_name);
                    setpurchaseNumber(response.data.data[0]?.sales_number);
                    settranseAmount(response.data.data[0]?.investment_amount);
                    setinvoiceDate(response.data.data[0]?.claims_date);
                    setinvoiceNumber(response.data.data[0]?.claims_non_ccis_number);
    ...
    const product_table = [
        ...
        { th: "Trade Discount", id: "other_discount_value", weigh: "30%", sub_item: [] },
