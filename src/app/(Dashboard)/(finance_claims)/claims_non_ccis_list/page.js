...
const OrderList = () => {
    ...
    const fetchData = async (
        ...
        axiosGet
            .get(
                `claims_non_ccis_list?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&claims_ccis_type=${2}`
            )
            .then((response) => {
    ...
    data?.map((item, index) => {
        const array_data = {
            ...
            data: [
                { td: item.claims_non_ccis_number, type: "text", id: 1, alian: "left" },
                {
    ...
    const HandleOpenViewInvoice = () => {
        router.push("/view_claims_non_ccis");
        handleClose2();
    };
    ...
    const handleCreatePurchaseInvoice = () => {
        router.push("/claims_non_ccis_create");
      };    
