...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `sales_return_reports_get?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&customer_id=${customer_id}&invoice_date=${invoice_date}&return_method=${2}`
      )
      .then((response) => {
  ...
  const tabs = [
    {
      value: 1,
      name: "Damaged Goods Reports",
      content: <TabContext value={1} />,
    },
    {
      value: 2,
      name: "Damaged Goods Item Reports",
