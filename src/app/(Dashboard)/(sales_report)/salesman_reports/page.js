...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `salesman_reports_get?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&sales_man_id=${salesman_id}&table_name=${table_name}`
      )
      .then((response) => {
  ...
  const handleSalesmanTabTwoChange = (event, value) => {
    if (value != null) {
      ...
      setSalesmanTabTwoName(value.full_name);
    } else {
      setSalesmanTabTwoID("");
            ...
            <Stack direction={"row"} gap={1}>
              <Box sx={{ mt: 1 }}>
                <Autocomplete
                  ...
                  value={
                    SalesmanMaster.find(
                      (year) => year.full_name === SalesmanTabTwoName
                    ) || null
                  }
                  onChange={(e, value) =>
  ...
  const tabs = [
    {
      value: 1,
      name: "Salesman Reports",
      content: <TabContext value={1} />,
    },
    {
      value: 2,
      name: "Salesman Item Reports",
