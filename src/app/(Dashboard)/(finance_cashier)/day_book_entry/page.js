...
const OrderList = () => {
  ...
  const handleCreatePurchaseInvoice = () => {
    router.push("/day_book_entry_create");
  };
  ...
  data?.map((item, index) => {
    const array_data = {
      ...
      data: [
        ...
        {
          td: item.employee_name == "" || item.employee_name == undefined ? 'NA':item.employee_name,
          type: "text",
          id: 6,
          alian: "left",
        },
        {
          td: item.narration_notes == "" || item.narration_notes == undefined ? "NA":item.narration_notes + ' As per invoice no: ' + item.sales_invoice_number,
          type: "text",
          id: 7,
          alian: "left",
        },{
          td: item.sales_invoice_number == "" || item.sales_invoice_number == undefined ? 'NA':item.sales_invoice_number,
          type: "text",
          id: 15,
          alian: "left",
        },
  ...
  const HandleVersionOrder = () => {
    const fetchData = async (access_token, ref_order_id) => {
      axiosGet
        .get(
          `day_book_version_list?access_token=${access_token}&ref_order_id=${ref_order_id}`
        )
        .then((res) => {
  ...
  const HandleOpenViewInvoiceManual = (value) => {
    ...
    router.push("/view_day_book_entry");
    HandleVersionOrderClose();
  };

  const HandleOpenEditInvoice = () => {
    router.push("/day_book_entry_edit");
    handleClose2();
  };
    ...
    return (
      <List sx={{ p: 0, fontSize: "12px" }}>
        ...
        <ListItemButton
          ...
          <Typography variant="p">Edit Entry</Typography>
        </ListItemButton>
        <Divider/>
