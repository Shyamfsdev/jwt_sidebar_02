...
import ProductCreate from "../../components/inwardentry_create/ProductCreateInward";
...
const EmployeeCreate = () => {
            ...
            if (item.balance_qty !== 0) {
              const order_list = {
                ...
                invoice_discount_per:
                  item.invoice_discount_value === undefined
                    ? ""
                    : item.invoice_discount_value,
                other_discount_per:
                  item.invoice_discount_value === undefined
                    ? ""
                    : item.invoice_discount_value,
  ...
  const product_table = [
    ...
    { th: "Received Qty", id: "return_qty", weigh: "12%", sub_item: [] },
  ...
  function isAccepted(items) {
    return items.every(item => parseInt(item.return_qty) > parseInt(item.material_qty));
  }
  ...
  const EmployeeCreateMaster = async () => {
    ...
    if (filteredOrderMaterial.length !== 0) {
      if (isAccepted(filteredOrderMaterial)) {
        const mdata = {
        ...
        setloadingPage(false);
        setError({ status: "error", message: "Material Quantity is Not Accepted!" });
      }
    } else {
      setloadingPage(false);
