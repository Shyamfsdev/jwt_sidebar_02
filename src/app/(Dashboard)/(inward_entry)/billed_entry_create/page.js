    ...
    const fetchData = async (access_token, data_uniq_id) => {
      axiosGet
        ...
        .then((response) => {
          ...
          setheadingTitle(
            Number(METHOD_TYPE) === 1 ? "Shortage" : "Undelivered"
          );
    ...
    if (filteredOrderMaterial.length !== 0) {
      ...
      axiosPost.post(`purchase_entry_master_action`, mdata).then((res) => {
        ...
        if (res.data.action_status === "Error") {
          ...
          const successMessage = "Inward Entry Created Successfully";
      ...
      >
        ...
        <Box sx={{ mb: 2, mt: 1 }}>
          <Typography
            ...
            color={"primary"}
          >
            Reference {headingTitle} Details
          </Typography>
        </Box>
