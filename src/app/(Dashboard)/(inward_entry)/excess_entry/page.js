...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `purchase_entry_master_list?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&pur_invoice_status=${pur_invoice_status}&inwaed_entry_method=${3}`
      )
      .then((response) => {
  ...
  const HandleClosedOrder = () => {
    if (
      ...
      setError({
        status: "error",
        message: "Excess Cannot be Return.",
      });
    }
  };
  ...
  const HandleCancelOrder = () => {
    if (
      ...
      router.push("/return_excess_entry");
    ...
    return (
      <List sx={{ p: 0, fontSize: "12px" }}>
        <ListItemButton onClick={() => HandleOpenViewInvoice()}>
          <Typography variant="p">View Excess</Typography>
        </ListItemButton>
        <ListItemButton
        ...
        >
          <Typography variant="p">Return Excess</Typography>
        </ListItemButton>
        <ListItemButton
        ...
        >
          <Typography variant="p">Close Excess</Typography>
        </ListItemButton>
      </List>
    );
  };
  ...
  const handleCreateInvoice = () => {
    if (invoiceOrderID !== "") {
      router.push("/excess_entry_create");
