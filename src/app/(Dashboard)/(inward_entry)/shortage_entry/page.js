...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `purchase_entry_master_list?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&pur_invoice_status=${pur_invoice_status}&inwaed_entry_method=${1}`
      )
      .then((response) => {
  ...
  const HandleOpenViewInvoice = () => {
    router.push("/view_shortage_entry");
    handleClose2();
  };
  ...
  const HandleClosedOrder = () => {
    if (
      ...
      setError({
        status: "error",
        message: "Shortage Cannot be Return.",
      });
    }
  };
    ...
    return (
      <List sx={{ p: 0, fontSize: "12px" }}>
        <ListItemButton onClick={() => HandleOpenViewInvoice()}>
          <Typography variant="p">View Shortage</Typography>
        </ListItemButton>
        <ListItemButton
        ...
        >
          <Typography variant="p">Close Shortage</Typography>
        </ListItemButton>
      </List>
    );
  };
  ...
  const handleCreateInvoice = () => {
    if (invoiceOrderID !== "") {
      router.push("/shortage_entry_create");
      ...
      return (
        <Box>
          ...
          >
            ...
            >
              <span>Direct Shortage Entry</span>
            </Button>
          </Box>
        </Box>
      );
