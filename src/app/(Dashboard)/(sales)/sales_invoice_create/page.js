...
import ProductCreate from "../../components/product_create/SalesProductCreate";
...
const EmployeeCreate = () => {
  ...
  const EmployeeCreateMaster = async () => {
    ...
    if (filteredOrderMaterial.length !== 0) {
      ...
      if (!sifyinvoiceNumber) {
        setError({
          status: "error",
          message: "Sify Invoice Number Is Required",
        });
        return;
      }
      if (!sifydocNumber) {
        setError({
          status: "error",
          message: "Sify Document Number Is Required",
        });
        return;
      }
      axiosPost.post(`sales_invoice_api`, mdata).then((res) => {
  ...
  const SalesinvoicePickCreateMaster = async () => {
    ...
    if (filteredOrderMaterial.length !== 0) {
      ...
      axiosPost.post(`sale_inv_against_insert_pick`, mdata).then((res) => {
        setloadingPage(false);
        if (res.data.action_status === "Error") {
          setError({ status: "error", message: res.data.message });
        } else {
  ...
  const TotalComponent = () => {
    return (
      <TotalAmount
        ...
        order_material={orderMaterial} state_user={0}
      />
    );
  };
  ...
  return (
    <div>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <Button
            className="nunito_font_width create_button_delivery"
            onClick={() => SalesinvoicePickCreateMaster()}
          >
            Save & Generate Picklist
          </Button>
          <Button
