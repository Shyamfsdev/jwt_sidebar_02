...
import SalesCreate from "../../components/product_view/WarehouseView";
...
const EmployeeCreate = () => {
  ...
  const GetFetData = () => {
    ...
    const fetchData = async (access_token, data_uniq_id) => {
      axiosGet
        .get(
          `warehouse_damage_entry_list?access_token=${access_token}&data_uniq_id=${data_uniq_id}`
        )
        .then((response) => {
            console.log(response);
          setinvoiceNumber(response.data.data[0]?.invoice_number);
          setinvoiceDate(response.data.data[0]?.entry_date);
  ...
  const product_table = [
    ...
    { th: "Damage Qty", id: "qty", weigh: "10%", sub_item: [] },
                ...
                >
                  ...
                  <div className="display_flex">
                    <span className="fontInput2">
                      Warehouse Damage Number : {invoiceNumber}
                    </span>
                  </div>
                  <div className="display_flex">
                    <span className="fontInput2">
