...
import AlertDialog from "../../../(Dashboard)/components/container/AlertDialogMultiple";
...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `sales_excel_get?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&sales_status=${sales_status}`
      )
      .then((response) => {
  ...
  const tableHead = [
    {
      id: 1,
      label: "Excel Number",
      value: "sales_excel_number",
    },
    {
      id: 2,
      label: "Period of Excel",
  ...
  data?.map((item, index) => {
    const array_data = {
      ...
      data: [
        { td: item.sales_excel_number, type: "text", id: 1, alian: "left" },
        {
          td:
            moment(item.sales_start_date).format("YYYY-MM-DD") +
            "  to  " +
            moment(item.sales_end_date).format("YYYY-MM-DD"),
          type: "text",
          id: 2,
          alian: "left",
        },
        ...
        //     <Box>
        //       <span
        //         className={`table_body_style_fab arrival_${item.sales_status_org}`}
        //         sx={{ width: "20%" }}
        //       >
        //         {item.sales_status}
        //       </span>
  ...
  const HandleCancelOrder = () => {
    if (singleData?.sales_status_org === 'waiting_for_arrival') {
      setcancelOrderStatus(true);
      handleClose2();
    } else {
      setError({ status: "error", message: "Sales Excel Cannot be Cancelled." });
    }
  };
  ...
  const DispatchCancelMaster = async () => {
    const mdata = {
      ...
      excel_order_id: singleData?.data_uniq_id,
      cancel_order: 1,
      cancel_notes: cancelOrderNotes,
    };
    axiosPost.post(`excel_order_edit`, mdata).then((res) => {
  ...
  const MenuComponent = () => {
    return (
      <List sx={{ p: 0, fontSize: "12px" }}>
        ...
        <ListItemButton onClick={() => HandleCancelOrder()} style={singleData?.sales_status_org === 'waiting_for_arrival' ? { opacity: 1 } : { opacity: 0.3 }}>
          <Typography variant="p">Cancel Excel</Typography>
        </ListItemButton>
      </List>
    );
  };
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        const data = {
          ...
          access_token: ACCESS_TOKEN,
          confirm_status:confirmModele === true ? 1:0
        };
        axiosPost
          .post("sales_excel_details", data)
          .then((response) => {
  ...
  const[confirmModele,setconfirmModele] = useState(false)
  ...
  const handleImportConfirm = () => {
    if (file) {
      ...
      reader.onload = () => {
        ...
        axiosPost
          .post("mismatch_excel_get", data)
          .then((response) => {
            console.log(response);
            if (response.data.message == true) {
              setconfirmModele(true);
              setconfirmMSG(response.data.reponse);
            } else {
              setconfirmModele(false)
              setconfirmMSG([])
              handleImport();
            }
          })
  ...
  return (
    <>
      <Box sx={{ px: 2, height: "1" }}>
        ...
        open={confirmModele}
        handleClose={ManualConfirmClose}
        text={confirmMSG}
      ></AlertDialog>
      </Box>
    </>
