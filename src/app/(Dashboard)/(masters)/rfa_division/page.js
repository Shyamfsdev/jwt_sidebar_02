...
const EmployeeList = () => {
    ...
    const toggleDrawer = (newOpen, type) => () => {
        if (newOpen === true) {
            if (type == 1) {
                ...
                setOpenDrawerType(type);
                setpageDrawerWidth(500);
                setjob_titleName(singleData.sub_division);
                setDataUniqId(singleData.data_uniq_id);
                setdepartmentName(singleData?.division);
                setdepartmentID(singleData?.division_id);
                handleClose2();
            }
        } else {
            setOpenDrawer(newOpen);
    ...
    const handleSubmit = () => {
        ...
        try {
            if (openDrawerType == 1) {
                axiosPost
                    .post(`sub_division_master`, jsonData)
                    .then((response) => {
                ...
                axiosPost
                    .put(`sub_division_master`, jsonData)
                    .then((response) => {
                        if (response.data.action === "success") {
                            setOpenDrawer(false);
                            setpageDrawerWidth(0);
    ...
    const fetchData = async () => {
        ...
        axiosGet
            .get(
                `sub_division_master_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${activeStatusFilter === 3 ? "" : activeStatusFilter
                }`
            )
            .then((response) => {
            ...
            <div style={{ height: "85vh", overflow: "auto" }}>
                <Box sx={{ my: 1, padding: "8px" }}>
                    <Autocomplete
                        ...
                        value={
                            departmentList.find(
                                (year) => year.division === departmentName
                            ) || null
                        }
                        onChange={(e, value) =>
