...
const EmployeeList = () => {
  ...
  const toggleDrawer = (newOpen, type) => () => {
    if (newOpen === true) {
      if (type == 1) {
        ...
        setOpenDrawerType(type);
        setpageDrawerWidth(500);
        setjob_titleName(singleData.employee_type);
  ...
  const handleSubmit = () => {
    const jsonData = {
      ...
      employee_type: job_titleName,
      active_status: dataStatus,
    };
    try {
      if (openDrawerType == 1) {
        axiosPost
          .post(`employee_type_master`, jsonData)
          .then((response) => {
        ...
        axiosPost
          .put(`employee_type_master`, jsonData)
          .then((response) => {
            if (response.data.action === "success") {
              setOpenDrawer(false);
              setpageDrawerWidth(0);
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `employee_type_details?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${
