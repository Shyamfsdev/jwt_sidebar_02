...
const EmployeeList = () => {
  ...
  const toggleDrawer = (newOpen, type) => () => {
    if (newOpen === true) {
      if (type == 1) {
        ...
        setOpenDrawerType(type);
        setpageDrawerWidth(500);
        setjob_titleName(singleData.type_of_spent);
        setDataUniqId(singleData.data_uniq_id);
        setopeningAmount(singleData.opening_amount);
        handleClose2();
      }
    } else {
      setOpenDrawer(newOpen);
  ...
  const handleSubmit = () => {
    const jsonData = {
      ...
      type_of_spent: job_titleName,
      opening_amount: openingAmount,
    };
    try {
      if (openDrawerType == 1) {
        axiosPost
          .post(`type_of_spent_api`, jsonData)
          .then((response) => {
        ...
        axiosPost
          .put(`type_of_spent_api`, jsonData)
          .then((response) => {
            if (response.data.action === "success") {
              setOpenDrawer(false);
              setpageDrawerWidth(0);
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `type_of_spent_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${
