...
const EmployeeList = () => {
  ...
  const toggleDrawer = (newOpen, type) => () => {
    if (newOpen === true) {
      if (type == 1) {
        ...
        setjob_titleName("");
        setDataUniqId("");
        setemployeeChooseValue(0);
      } else {
        setOpenDrawer(newOpen);
        setOpenDrawerType(type);
        setpageDrawerWidth(500);
        setjob_titleName(singleData.account_head_type);
        setDataUniqId(singleData.data_uniq_id);
        setemployeeChooseValue(singleData?.employee_status);
        handleClose2();
      }
    } else {
      setOpenDrawer(newOpen);
  ...
  const handleSubmit = () => {
    const jsonData = {
      ...
      account_head_type: job_titleName,
      active_status: dataStatus,
      employee_status: employeeChooseValue
    };
    try {
      if (openDrawerType == 1) {
        axiosPost
          .post(`account_head_master`, jsonData)
          .then((response) => {
        ...
        axiosPost
          .put(`account_head_master`, jsonData)
          .then((response) => {
            if (response.data.action === "success") {
              setOpenDrawer(false);
              setpageDrawerWidth(0);
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `account_head_master_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${activeStatusFilter === 3 ? "" : activeStatusFilter
        }`
      )
      .then((response) => {
  ...
  const title = "Account Head";

  const tableHead = [
    {
      id: 1,
      label: `${title} Name`,
      value: "account_head_type",
    },
    {
      id: 2,
      label: "Created Date",
  ...
  const [employeeChooseValue, setemployeeChooseValue] = useState(0);

  const ChangeFreeStock = (event, value) => {
    if (value === 0) {
      setemployeeChooseValue(1);
    } else {
      setemployeeChooseValue(0);
    }
  };
    ...
    return (
      <div style={{ height: "85vh", overflow: "auto" }}>
        <Stack direction={"row"} gap={2}>
          <Box sx={{ my: 1, padding: "10px", width: "60%" }}>
            <TextField
              id="outlined-basic"
              label="Account Head"
              variant="outlined"
          ...
          <Box sx={{ my: 1, px: 1, width: "40%" }} className="flex_display">
            <div>
              <Checkbox
                onChange={(event) => ChangeFreeStock(event, employeeChooseValue)}
                checked={employeeChooseValue == 0 ? false : true}
          ...
          <div style={{ padding: "10px" }}>
            ...
            >
              ...
              >
                ...
                <TableBody>
                  {singleData?.account_histroy?.map((row, index) => (
                    <TableRow key={row.id}>
                      <TableCell sx={{ padding: "4px 8px" }} align="center">
                        {row.entry_date}
                      </TableCell>
                      <TableCell sx={{ padding: "4px 8px" }} align="center">
                        {singleData?.employee_status === "" || singleData?.employee_status === undefined
                          ? "NA"
                          : row.employee_name}
                      </TableCell>
                      <TableCell sx={{ padding: "4px 8px" }} align="center">
                      {singleData?.narration_notes === "" || singleData?.narration_notes === undefined
                          ? "NA"
                          : row.narration_notes}
                      </TableCell>
                      <TableCell sx={{ padding: "4px 8px" }} align="center">
                      {singleData?.sales_invoice_number === "" || singleData?.sales_invoice_number === undefined
                          ? "NA"
                          : row.sales_invoice_number}
                      </TableCell>
                      <TableCell sx={{ padding: "4px 8px" }} align="center">
                      {singleData?.entry_by === undefined ? "NA":singleData?.entry_by[0]?.first_name} {singleData?.entry_by !== undefined && singleData?.entry_by[0]?.last_name}
                      </TableCell>
                      <TableCell sx={{ padding: "4px 8px" }} align="center">
                        {row.amount}
                      </TableCell>
