...
const EmployeeList = () => {
  ...
  const toggleDrawer = (newOpen, type) => () => {
    if (newOpen === true) {
      if (type == 1) {
        ...
        setOpenDrawerType(type);
        setpageDrawerWidth(500);
        setjob_titleName(singleData.desig_name);
        setDataUniqId(singleData.data_uniq_id);
        setdepartmentName(singleData?.department_list?.department_name);
        setdepartmentID(singleData?.department_list?.data_uniq_id);
        handleClose2();
      }
    } else {
      setOpenDrawer(newOpen);
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `designation_details?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${
  ...
  const ChangeDepartment = (e, v) => {
    if (v != null) {
      setdepartmentName(v.department_name);
  ...
  const CreateCompannet = () => {
    ...
    const DepartmentMasterGet = () => {
      const fetchData = async (
        ...
        axiosGet
          .get(
            `department_details?access_token=${access_token}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}`
          )
          .then((res) => {
      ...
      <div style={{ height: "85vh", overflow: "auto" }}>
        <Box sx={{ my: 1, padding: "10px" }}>
          <Autocomplete
            ...
            value={
              departmentList.find(
                (val) => val.department_name === departmentName
              ) || null
            }
            onChange={(e, value) => ChangeDepartment(e.target.value, value)}
          ...
          <TextField
            ...
            size="small"
            fullWidth
            placeholder="Salesman Designation"
            sx={{ pb: 1 }}
            inputProps={{
              style: {
                fontSize: "12px",
