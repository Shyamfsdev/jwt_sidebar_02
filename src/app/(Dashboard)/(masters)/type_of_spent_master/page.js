...
import MasterDrawer from "../../components/MasterDrawer/Sidebar";
...
const EmployeeList = () => {
    ...
    const toggleDrawer = (newOpen, type) => () => {
        if (newOpen === true) {
            if (type == 1) {
                ...
                setOpenDrawerType(type);
                setpageDrawerWidth(500);
                setjob_titleName(singleData.sub_type_of_spent);
                setDataUniqId(singleData.data_uniq_id);
                setclaimsID(singleData?.type_of_spent_id);
                setclaimsName(singleData?.type_of_spent);
                handleClose2();
            }
        } else {
            setOpenDrawer(newOpen);
    ...
    const handleSubmit = () => {
        const jsonData = {
            ...
            sub_type_of_spent: job_titleName,
            type_of_spent_id: claimsID,
            type_of_spent:claimsName
        };
        try {
            if (openDrawerType == 1) {
                axiosPost
                    .post(`sub_type_of_spent_api`, jsonData)
                    .then((response) => {
                ...
                axiosPost
                    .put(`sub_type_of_spent_api`, jsonData)
                    .then((response) => {
                        if (response.data.action === "success") {
                            setOpenDrawer(false);
                            setpageDrawerWidth(0);
    ...
    const [typeOFClaimMaster, settypeOFClaimMaster] = useState([]);

    const HandleClaimsMaster = () => {
        const fetchData = async (
            ...
            axiosGet
                .get(
                    `type_of_spent_list?access_token=${access_token}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&active_status=${1}`
                )
                .then((res) => {
                    if (res.data.action == "success") {
                        settypeOFClaimMaster(res.data.data);
                    }
                });
        };
    ...
    const fetchData = async () => {
        ...
        axiosGet
            .get(
                `sub_type_of_spent_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${activeStatusFilter === 3 ? "" : activeStatusFilter
                }`
            )
            .then((response) => {
    ...
    const title = "Type Of Spent";

    const tableHead = [
        {
            id: 1,
            label: `${title}`,
            value: "sub_type_of_spent",
        },
        {
            id: 2,
            label: "Type Of Claim",
    ...
    const [claimsName, setclaimsName] = useState("");

    const HandleClaims = (event, value) => {
        if (value !== null) {
            setclaimsID(value?.data_uniq_id);
            setclaimsName(value?.type_of_spent);
        } else {
            setclaimsID("");
            setclaimsName("");
        }
            ...
            <div style={{ height: "85vh", overflow: "auto" }}>
                <Box sx={{ my: 1, padding: "10px" }}>
                    <Autocomplete
                        ...
                        options={typeOFClaimMaster}
                        value={
                            typeOFClaimMaster?.find(
                                (year) => year.type_of_spent === claimsName
                            ) || null
                        }
                        onChange={(e, value) =>
                            HandleClaims(e.target.value, value)
                        }
                        getOptionLabel={(val) => val.type_of_spent}
                        required
                        id="type_of_claim"
                        renderInput={(params) => (
                            <TextField
