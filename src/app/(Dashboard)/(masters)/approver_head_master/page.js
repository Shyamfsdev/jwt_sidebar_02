...
const EmployeeList = () => {
  ...
  const toggleDrawer = (newOpen, type) => () => {
    if (newOpen === true) {
      if (type == 1) {
        ...
        setOpenDrawerType(type);
        setpageDrawerWidth(500);
        setjob_titleName(singleData.scam_approval_head);
  ...
  const handleSubmit = () => {
    const jsonData = {
      ...
      scam_approval_head: job_titleName,
    };
    try {
      if (openDrawerType == 1) {
        axiosPost
          .post(`scam_approval_head_api`, jsonData)
          .then((response) => {
        ...
        axiosPost
          .put(`scam_approval_head_api`, jsonData)
          .then((response) => {
            if (response.data.action === "success") {
              setOpenDrawer(false);
              setpageDrawerWidth(0);
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `scam_approval_head_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${
