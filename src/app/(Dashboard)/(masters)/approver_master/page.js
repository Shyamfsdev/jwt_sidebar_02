...
const EmployeeList = () => {
    ...
    const toggleDrawer = (newOpen, type) => () => {
        if (newOpen === true) {
            if (type == 1) {
                ...
                setOpenDrawerType(type);
                setpageDrawerWidth(500);
                setjob_titleName(singleData.scam_approval);
                setDataUniqId(singleData.data_uniq_id);
                setclaimsID(singleData?.scam_approval_head_id);
                setclaimsName(singleData?.scam_approval_head);
                handleClose2();
            }
        } else {
            setOpenDrawer(newOpen);
    ...
    const handleSubmit = () => {
        const jsonData = {
            ...
            scam_approval: job_titleName,
            scam_approval_head_id: claimsID,
            scam_approval_head:claimsName
        };
        try {
            if (openDrawerType == 1) {
                axiosPost
                    .post(`scam_approval_api`, jsonData)
                    .then((response) => {
                ...
                axiosPost
                    .put(`scam_approval_api`, jsonData)
                    .then((response) => {
                        if (response.data.action === "success") {
                            setOpenDrawer(false);
                            setpageDrawerWidth(0);
    ...
    const [approverHeadMaster, setapproverHeadMaster] = useState([]);
    ...
    const HandleClaimsMaster = () => {
        const fetchData = async (
            ...
            axiosGet
                .get(
                    `scam_approval_head_list?access_token=${access_token}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&active_status=${1}`
                )
                .then((res) => {
                    if (res.data.action == "success") {
                        setapproverHeadMaster(res.data.data);
                    }
                });
        };
    ...
    const fetchData = async () => {
        ...
        axiosGet
            .get(
                `scam_approval_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${activeStatusFilter === 3 ? "" : activeStatusFilter
                }`
            )
            .then((response) => {
    ...
    const HandleClaims = (event, value) => {
        if (value !== null) {
            ...
            setclaimsName(value?.scam_approval_head);
        } else {
            setclaimsID("");
            setclaimsName("");
        }
            ...
            <div style={{ height: "85vh", overflow: "auto" }}>
                <Box sx={{ my: 1, padding: "10px" }}>
                    <Autocomplete
                        ...
                        options={approverHeadMaster}
                        value={
                            approverHeadMaster?.find(
                                (year) => year.scam_approval_head === claimsName
                            ) || null
                        }
                        onChange={(e, value) =>
                            HandleClaims(e.target.value, value)
                        }
                        getOptionLabel={(val) => val.scam_approval_head}
                        required
                        id="approver_head"
                        renderInput={(params) => (
                            <TextField
