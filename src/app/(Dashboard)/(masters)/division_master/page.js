...
const EmployeeList = () => {
  ...
  const toggleDrawer = (newOpen, type) => () => {
    if (newOpen === true) {
      if (type == 1) {
        ...
        setjob_titleName("");
        setDataUniqId("");
        setcigrateStockValue(0);
        setstickProductValue(0);
        setpriorityTypeMethod(0);
      } else {
        setOpenDrawer(newOpen);
        setOpenDrawerType(type);
        setpageDrawerWidth(500);
        setjob_titleName(singleData.division);
        setDataUniqId(singleData.data_uniq_id);
        setcigrateStockValue(singleData.cigarette_division);
        setstickProductValue(singleData?.stick_status);
        setstickProductValue(singleData?.priority);
        handleClose2();
      }
    } else {
      setOpenDrawer(newOpen);
  ...
  const [priorityTypeMethod, setpriorityTypeMethod] = useState("");
  console.log(priorityTypeMethod,"PRIORITY METHOD")
 
  // Funtion for create new data or edit existing data
  const handleSubmit = () => {
    let priorityMethodValue = 0;
      if (priorityTypeMethod === "First Priority") {
        priorityMethodValue = 1;
      } else if (priorityTypeMethod === "Second Priority") {
        priorityMethodValue = 2;
      } else if (priorityTypeMethod === "Third Priority") {
        priorityMethodValue = 3;
      }
    const jsonData = {
      access_token: ACCESS_TOKEN,
      data_uniq_id: dataUniqId,
      division: job_titleName,
      cigarette_division:cigrateStockValue,
      active_status: dataStatus,
      stick_status: stickProductValue,
      priority: priorityMethodValue
    };
    try {
      if (openDrawerType == 1) {
        axiosPost
          .post(`division_master`, jsonData)
          .then((response) => {
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `division_master_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${
  ...
  const [cigrateStockValue, setcigrateStockValue] = useState(0);

  const ChangeFreeStock = (event, value) => {
    if (value === 0) {
      setcigrateStockValue(1);
    } else {
      setcigrateStockValue(0);
    }
  };

  const [stickProductValue, setstickProductValue] = useState(0);
  console.log(stickProductValue,"DATATA")
  const changeStickProduct = (event) => {
    const newValue = event.target.checked ? 1 : 0;
    setstickProductValue(newValue);
  };
        ...
        <Box sx={{ my: 1, padding: "5px" }}>
          <div className="master_create_style">
            ...
            <Stack direction={"row"} gap={2}>
              ...
              <Box sx={{ my: 1, px: 1, width:"25%"}} className="flex_display">
                <div>
                  <Checkbox
                    onChange={(event) => ChangeFreeStock(event, cigrateStockValue)}
                    checked={cigrateStockValue == 0 ? false : true}
              ...
              <div>
                  <Checkbox
                    onChange={changeStickProduct}
                    checked={stickProductValue === 1}
                    style={{ padding: '0px', color: '#c3c3c3' }}
                  />
                  <Typography
                    variant="p"
            ...
            <Stack direction={"row"} gap={1}> 
              <Box sx={{ my: 1, px: 1, width:'40%'}}>
                <Autocomplete
                  margin="normal"
                  variant="outlined"
                  style={{ marginTop: "8px" }}
                  options={["First Priority", "Second Priority", "Third Priority"]}
                  getOptionLabel={(val) => val}
                  required
                  id="priority-type"
                  onChange={(event, value) => setpriorityTypeMethod(value)}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      margin="normal"
                      value={priorityTypeMethod}
                      style={{ margin: "0px" }}
                      InputLabelProps={{
