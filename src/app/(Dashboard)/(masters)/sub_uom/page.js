...
const EmployeeList = () => {
  ...
  const toggleDrawer = (newOpen, type) => () => {
    if (newOpen === true) {
      if (type == 1) {
        ...
        setOpenDrawerType(type);
        setpageDrawerWidth(500);
        setjob_titleName(singleData.sub_uom);
  ...
  const handleSubmit = () => {
    const jsonData = {
      ...
      sub_uom: job_titleName,
      active_status: dataStatus,
    };
    try {
      if (openDrawerType == 1) {
        axiosPost
          .post(`sub_uom_master`, jsonData)
          .then((response) => {
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `sub_uom_master_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${
  ...
  const ChangeCgstValue = (event, value, index,status) => {
    ...
    if(status === false){
      if (value !== null || value !== undefined) {
        newFields[index].uom_id = value.data_uniq_id;
        newFields[index].uom_name = value.uom;
      } else {
        newFields[index].uom_id = "";
        newFields[index].uom_name = "";
      }
    }else{
      newFields[index].uom_id = "";
      newFields[index].uom_name = "";
