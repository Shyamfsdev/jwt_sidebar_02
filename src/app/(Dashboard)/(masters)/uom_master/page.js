...
const EmployeeList = () => {
  ...
  const toggleDrawer = (newOpen, type) => () => {
    if (newOpen === true) {
      ...
      if (type === 1) {
        setjob_titleName("");
        // Initialize with non-empty entries
        const newTaxDetails = data.map(() => ({
          uom_id: "",
          uom_name: "",
        }));
        settaxDetails(newTaxDetails.filter(item => item.uom_id && item.uom_name));
        setDataUniqId("");
      } else {
        setjob_titleName(singleData.uom);
        setDataUniqId(singleData.data_uniq_id);
        console.log(singleData,"SDARR")
        settaxDetails(singleData.subuom_details.filter(item => item.uom_id && item.uom_name));
        handleClose2();
      }
    } else {
      setOpenDrawer(newOpen);
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `uom_master_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${
  ...
  const ActionComponent = () => {
    return (
      ...
      const [SubuomMaster, setSubuomMaster] = useState([]);
      const HandleSubuomMaster = () => {
        const fetchData = async (
          access_token,
          searchValue,
          createdStartDate,
          ...
          axiosGet
            .get(
              `sub_uom_master_list?access_token=${access_token}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}`
            )
            .then((res) => {
              if (res.data.action == "success") {
                setSubuomMaster(res.data.data);
              }
            });
        };
  ...
  const ChangeCgstValue = (event, value, index, status) => {
    let newFields = [...taxDetails];
  
    if (status === false) {
      if (value !== null && value !== undefined && !newFields.some(item => item.uom_id === value.data_uniq_id)) {
        newFields.push({ uom_id: value.data_uniq_id, uom_name: value.sub_uom });
      }
    } else {
      newFields = newFields.filter(item => item.uom_id !== value.data_uniq_id);
    }
    newFields = newFields.filter(item => item.uom_id && item.uom_name);
                    ...
                    <Checkbox
                      onChange={(event) =>
                        ChangeCgstValue(
                          ...
                          index,
                          taxDetails.some(
                            (vale) => res.data_uniq_id === vale.uom_id
                          )
                        )
                      }
                      checked={taxDetails.some(
                        (vale) => res.data_uniq_id === vale.uom_id
                      )}
