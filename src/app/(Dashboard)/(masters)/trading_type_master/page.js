...
const EmployeeList = () => {
  ...
  const toggleDrawer = (newOpen, type) => () => {
    ...
    if (newOpen === true) {
      if (type == 1) {
        ...
        setOpenDrawerType(type);
        setpageDrawerWidth(500);
        setjob_titleName(singleData.trading_type);
  ...
  const handleSubmit = () => {
    const jsonData = {
      ...
      trading_type: job_titleName,
      active_status: dataStatus,
    };
    try {
      if (openDrawerType == 1) {
        axiosPost
          .post(`trading_type_master`, jsonData)
          .then((response) => {
        ...
        axiosPost
          .put(`trading_type_master`, jsonData)
          .then((response) => {
            if (response.data.action === "success") {
              setOpenDrawer(false);
              setpageDrawerWidth(0);
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `trading_type_master_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${
