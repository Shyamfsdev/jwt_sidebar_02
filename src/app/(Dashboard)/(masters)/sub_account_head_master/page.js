...
const EmployeeList = () => {
    ...
    const toggleDrawer = (newOpen, type) => () => {
        if (newOpen === true) {
            if (type == 1) {
                ...
                setOpenDrawerType(type);
                setpageDrawerWidth(500);
                setjob_titleName(singleData.sub_account_head_type);
                setDataUniqId(singleData.data_uniq_id);
                setdepartmentName(singleData?.account_head);
                setdepartmentID(singleData?.account_head_id);
                handleClose2();
            }
        } else {
            setOpenDrawer(newOpen);
    ...
    const handleSubmit = () => {
        const jsonData = {
            ...
            sub_account_head_type: job_titleName,
            active_status: dataStatus,
            account_head_id: departmentID,
            account_head:departmentName
        };
        try {
            if (openDrawerType == 1) {
                axiosPost
                    .post(`sub_account_head_master`, jsonData)
                    .then((response) => {
                ...
                axiosPost
                    .put(`sub_account_head_master`, jsonData)
                    .then((response) => {
                        if (response.data.action === "success") {
                            setOpenDrawer(false);
                            setpageDrawerWidth(0);
    ...
    const fetchData = async () => {
        ...
        axiosGet
            .get(
                `sub_account_head_master_list?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${activeStatusFilter === 3 ? "" : activeStatusFilter
                }`
            )
            .then((response) => {
    ...
    const title = "Sub Account Head";

    const tableHead = [
        {
            id: 1,
            label: `${title} Name`,
            value: "sub_account_head_type",
        },
        {
            id: 2,
            label: `Account Head`,
    ...
    const ChangeDepartment = (e, v) => {
        if (v != null) {
            setdepartmentName(v.account_head_type);
            setdepartmentID(v.data_uniq_id);
        } else {
            setdepartmentName("");
            setdepartmentID("");
        }
    ...
    const CreateCompannet = () => {
        ...
        const DepartmentMasterGet = () => {
            const fetchData = async (
                ...
                axiosGet
                    .get(
                        `account_head_master_list?access_token=${access_token}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&employee_status=${0}`
                    )
                    .then((res) => {
            ...
            <div style={{ height: "85vh", overflow: "auto" }}>
                <Box sx={{ my: 1, padding: "8px" }}>
                    <Autocomplete
                        ...
                        value={
                            departmentList.find(
                                (year) => year.account_head_type === departmentName
                            ) || null
                        }
                        onChange={(e, value) =>
                            ChangeDepartment(e.target.value, value)
                        }
