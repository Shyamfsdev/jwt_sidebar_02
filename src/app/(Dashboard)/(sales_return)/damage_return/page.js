...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `sales_return_master_list?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&return_method=2`
      )
      .then((response) => {
  ...
  const handleCreateInvoice = () => {
    if (invoiceOrderID !== "" && materialCode != "") {
      router.push("/damage_goods_against_sales");
  ...
  const handleCreateEntry = () => {
    router.push("/diract_damage_goods");
  };
  ...
  const handleInvoiceOrderChange = (event, value) => {
    if (value != null) {
      ...
      setinvoiceOrderNumber(value.customer_name);
          ...
          <Stack direction={"row"} gap={2} sx={{ width: "100%" }}>
            <Box sx={{ my: 1, width: "100%" }}>
              <Autocomplete
                ...
                value={
                  customerMaster.find(
                    (year) => year.customer_name === invoiceOrderNumber
                  ) || null
                }
                onChange={(e, value) =>
