...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `sales_return_master_list?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&return_method=1`
      )
      .then((response) => {
  ...
  const tableHead = [
    ...
    },
    {
      ...
      id: 10,
      label: "Receiver Customer Name",
      value: "ref_customer_name",
    },
    {
      id: 4,
      label: "Contact No.",
  ...
  const HandleOpenViewInvoice = () => {
    router.push("/view_customer_replacement");
  ...
  const handleCreateInvoice = () => {
    if (invoiceOrderID !== "") {
      router.push("/customer_replacement_create");
      ...
      return (
        <Box>
          ...
          >
            ...
            >
              <span>Direct Invoice Replacement</span>
            </Button>
          </Box>
        </Box>
      );
    ...
    <>
      <Box sx={{ px: 2, height: "1" }}>
        <div className="dispatch_do_flex">
          <CreateButton
            heading={"Invoice Replacement List"}
