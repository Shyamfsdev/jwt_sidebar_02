...
const EmployeeCreate = () => {
    ...
    const fetchData = async (access_token, data_uniq_id) => {
      axiosGet
        ...
        .then((response) => {
          ...
          setdriverDetails(
            response.data.data[0]?.pick_list_details[0]?.vehicle_details
              ?.driver_name
          );
          setCompanyName(
            response.data.data[0]?.pick_list_details[0]?.vehicle_details
          ...
          response.data.data[0]?.invoice_material_details.map((item) => {
            if (ITEM_CODE === item.material_code) {
              const order_list = {
                ...
                batch_list:
                  item.batch_list === undefined ? [] : item.batch_list,
                free_stock: item.free_stock === undefined ? 0 : item.free_stock,
                new_batch_value:
                  item.new_batch_value === undefined
                    ? ""
                    : item.new_batch_value,
  ...
  const HandleInvoiceMaster = () => {
    const fetchData = async (
      ...
      axiosGet
        .get(
          `sales_invoice_details_get?access_token=${access_token}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&sal_entry_status=${"picklist not generated"}&return_status=0&customer_id=${customer_id}&item_code=${item_code}`
        )
        .then((res) => {
          if (res.data.action == "success") {
            setinvoiceOrderDetails(res.data.data);
            setcustomerID(res.data.data[0]?.customer_id);
            setcustomerName(res.data.data[0]?.customer_name);
            setcustomerDataValue(res.data.data[0]?.customer_data);
            setcustomerAddress(res.data.data[0]?.customer_data?.address);
            setcustomerEmail(res.data.data[0]?.customer_data?.email_id);
            setcustomerGst(res.data.data[0]?.customer_data?.gst_no);
            setcustomerMobile(res.data.data[0]?.customer_data?.contact_number);
          }
        });
    };
  ...
  const product_table = [
    ...
    { th: "Damage Quantity", id: "return_qty", weigh: "6%", sub_item: [] },
  ...
  const EmployeeCreateMaster = async () => {
    ...
    if (filteredOrderMaterial.length !== 0) {
      ...
      const mdata = {
        ...
        sales_invoice_id: invoiceOrderID,
  ...
  const SupplierDetailsComponent = () => {
    return (
      ...
      >
        ...
        {invoiceOrderID !== "" && invoiceOrderID !== undefined && (
        <>
            <Box sx={{ mb: 2, mt: 2 }}>
              <Typography
                variant="p"
