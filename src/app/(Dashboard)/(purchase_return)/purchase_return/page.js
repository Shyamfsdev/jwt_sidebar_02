...
const OrderList = () => {
  ...
  const fetchData = async (
    ...
    axiosGet
      .get(
        `purchase_return_master_list?access_token=${access_token}&page=${limit}&items_per_page=${end}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${order_type}&order_field=${order_field}&pur_invoice_status=${pur_invoice_status}&delivery_challan=0`
      )
      .then((response) => {
    ...
    const array_data = {
      ...
      data: [
        ...
        },
        {
          ...
          td: item.purchase_entry_return_againt === 0 ? 'NA':item.pur_invoice_number?.pur_invoice_number,
          type: "text",
          id: 6,
          alian: "left",
        },{
    ...
    return (
      <List sx={{ p: 0, fontSize: "12px" }}>
        ...
        <ListItemButton
          ...
          style={
            singleData?.pur_return_status_org === "waiting_for_arrival"
              ? { opacity: 1 }
              : { opacity: 0.3 }
          }
        >
          <Typography variant="p">Cancel Return</Typography>
        </ListItemButton>
      </List>
    );
  };
  ...
  const HandleOpenViewInvoice = () => {
    router.push("/view_purchase_return");
  ...
  const handleCreateInvoice = () => {
    if (invoiceOrderID !== "") {
      router.push("/purchase_entry_against_return");
      Cookies.set("data_uniq_id", invoiceOrderID);
      Cookies.set("item_code", materialCode);
    } else {
  ...
  const handleCreateEntry = () => {
    router.push("/purchase_direct_return");
  };
  ...
  const handleInvoiceOrderChange = (event, value) => {
    if (value != null) {
      ...
      setinvoiceOrderNumber(value.supplier_name);
          ...
          <Box sx={{ display: "flex", mb: 2 }}>
            ...
            <Box sx={{ my: 1, width: "100%" }}>
              <Autocomplete
                ...
                value={
                  supplierMaster.find(
                    (year) => year.supplier_name === invoiceOrderNumber
                  ) || null
                }
                onChange={(e, value) =>
