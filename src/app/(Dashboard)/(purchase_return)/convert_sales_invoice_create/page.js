...
import ProductCreate from "../../components/product_create/DeliverySalesInvoice";
...
const EmployeeCreate = () => {
  ...
  const DATA_UNIQ_ID = Cookies.get("invoice_id");
  ...
  const fitchDataValue = () => {
    setloadingPage(true);
    const fetchData = async (access_token, data_uniq_id) => {
      axiosGet
        .get(
          `purchase_return_master_list?access_token=${access_token}&pur_invoice_id=${data_uniq_id}`
        )
        .then((response) => {
          ...
          if (data) {
            ...
            setcustomerDataValue(data.customer_data);
            setcustomerAddress(data.customer_data?.address);
            setcustomerEmail(data.customer_data?.email_id);
            setcustomerGst(data.customer_data?.gst_no);
            setcustomerMobile(data.customer_data?.contact_number);
            setRouteName(data.route_name);
            setRouteID(data.route_id);
            setsalesManName(data.sales_man_name);
            setsalesManID(data.sales_man_id);
            setcustomerSalesName(
              data.customer_data?.route_details[0]?.salesManName
            );
            setcustomerRoute(data.customer_data?.route_details[0]?.routeName);
            setDivision(data.customer_data?.route_details[0]?.divisionName);

            let order_material = data.material_list
              .filter((item) => item.balance_qty !== 0)
              .map((item) => ({
  ...
  const product_table = [
    ...
    { th: "DC Quantity", id: "qty", weigh: "10%", sub_item: [] },
    { th: "Material Quantity", id: "return_qty", weigh: "10%", sub_item: [] },
    ...
    if (value !== undefined && value !== null) {
      ...
      if (ref === "material_code") {
        ...
        if (!isNaN(event.target.value) && event.target.value >= 0) {
          newfield[index].material_qty = Number(event.target.value);
          const total_amount =
  ...
  const EmployeeCreateMaster = async () => {
    ...
    if (filteredOrderMaterial.length !== 0) {
      ...
      const mdata = {
        ...
        ref_return_order_id: DATA_UNIQ_ID,
        customer_id: customerID,
        customer_name: customerName,
        customer_data: customerDataValue,
        sify_invoice_number: sifyinvoiceNumber,
        sify_doc_number: sifydocNumber,
      ...
      axiosPost.post(`sales_invoice_api_in_dc`, mdata).then((res) => {
        setloadingPage(false);
        if (res.data.action === "Error") {
          setError({ status: "error", message: "Data Error" });
        } else {
          const successMessage = "Sales Invoice Created Successfully";
      ...
      axiosPost.post(`sales_invoice_api_in_dc`, mdata).then((res) => {
        ...
        if (res.data.action === "Error") {
          ...
          setTimeout(() => {
            ...
            Cookies.set("invoice_id", DATA_UNIQ_ID);
          }, 200);
        }
      });
    } else {
  ...
  const HandleCreateConfirm = () => {
    ...
    for (let i = 0; i < orderMaterial.length; i++) {
      if (orderMaterial[i].dc_qty !== orderMaterial[i].material_qty) {
        return_status = true;
        break;
      }
    }
    if (return_status === false) {
      ...
      setcreateConfirmStatus((prevStatus) => !prevStatus);
    }
  };
      ...
      >
        ...
        <Box sx={{ margin: "8px 0px" }}>
          <TextField
            ...
            onChange={(e) => setsifyinvoiceNumber(e.target.value)}
        ...
        <Box sx={{ margin: "8px 0px" }}>
          <TextField
            ...
            size="small"
            value={sifydocNumber}
            onChange={(e) => setsifydocNumber(e.target.value)}
