...
const EmployeeCreate = () => {
  ...
  const GetFetData = () => {
    ...
    const fetchData = async (access_token, purchase_order_entry_id) => {
      axiosGet
        .get(
          `purchase_order_entry_id?access_token=${access_token}&pur_entry_number=${purchase_order_entry_id}`
        )
        .then((response) => {
          ...
          response.data.data[0]?.material_list.map((item) => {
            if (item.balance_qty !== 0) {
              const order_list = {
                ...
                ref_material_id: item.pur_entry_material_id
              };
            ...
            <Paper sx={{ p: 1, mb: 1 }}>
              ...
              <Stack direction={"row"} gap={2}>
                <Box
                  ...
                  sx={{ px: 1, width: "27%", marginTop: "10px" }}
                >
                  <Box sx={{ mb: 2, mt: 1 }}>
                    <Typography
                      variant="p"
